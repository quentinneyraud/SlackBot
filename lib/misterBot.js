...
misterBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        !this._isFromMisterBot(message) &&
        this._isMentioningMisterBot(message)
    ) {
        this.message = message;
        this.message.channelName = this._getChannelById(this.message.channel).name;
        this.message.userName = this._getUserById(this.message.user).name;
        this._getRequestType();
    }
};

misterBot.prototype._isChatMessage = function (message) {
    return message.type === 'message' && Boolean(message.text);
};

misterBot.prototype._isChannelConversation = function (message) {
    return typeof message.channel === 'string' &&
        message.channel[0] === 'C'
        ;
};

misterBot.prototype._isMentioningMisterBot = function (message) {
    return message.text.toLowerCase().indexOf(this.user.name) > -1;
};

misterBot.prototype._isFromMisterBot = function (message) {
...
misterBot.prototype._getRequestType = function() {
    ...
    if(!hasCatchRegex){
        this._respond("He! On m'a appelé mais j'ai pas compris votre requête.\nRegardez ce que je peux faire : \n" + this.requestsMan);
    }
};
...
misterBot.prototype._respond = function(message) {
    this.postMessageToChannel(this.message.channelName, message, {as_user: true});
};
...
misterBot.prototype._setMan = function(){
    ...
    this.requestsConfig.forEach(function(request){
        self.requestsMan += request.stringCatch + " : " + request.description + "\n";
    });
}
